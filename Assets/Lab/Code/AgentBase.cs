...
public class AgentBase : MonoBehaviour, IClickable
{
    //Interface implementation
    void IClickable.Clicked(Ray vRay, RaycastHit vHit, bool vDown)
    {
        Debug.LogFormat("Click {0} {1}", vHit.collider, gameObject.name, vDown);
        if (vDown)
        {
            Selected = !Selected;
        }
    }
    void IClickable.Hover(Ray vRay, RaycastHit vHit)
    {
        Debug.LogFormat("Hover {0}", vHit.collider, gameObject.name);
    }
    ...
    bool mSelected = false; //Keep track of if we are selected
    ...
    protected virtual bool   Perception()
    {
        ...
        if(Physics.Raycast(tForwardRay, out tHit, RayLenght))
        {
            if (mOnAgentStatusChange != null) //Is there a callback delegate
            {
                ...
                {
                    if (tHit.collider.GetComponent<AgentBase>() != null) //Only avoid other agents
                    {
                        ClearPath();
                        if (mCheckProgress != null)
                        {
    ...
    public  void SetDestination(Vector3 vDestination)
    {
        SetDestination(vDestination, OnAgentResult);
    }
